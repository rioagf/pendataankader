        ...
        <div style="background-color: #ffffff !important; padding: 25px; margin-bottom: 100px; overflow: auto;">
            <form action="<?php echo $action; ?>" method="post">
                ...
                <div class="form-group">
                    ...
                    <select class="selectnamapengaju form-control" name="nama_pembuat_pengajuan" required="">
                        <optgroup label="Pilih Nama">
                            <?php foreach ($data_warga as $warga) { ?>
                                <option value="<?= $warga->nama_lengkap ?>"><?= $warga->nama_lengkap ?></option>
                            <?php } ?>
                            
                        </optgroup>
                    </select>
                    <!-- <input type="text" class="form-control" name="nama_pembuat_pengajuan" id="nama_pembuat_pengajuan" placeholder="Nama Pembuat Pengajuan" required="" /> -->
                </div>
                <div id="kematian">
                    <div class="form-group">
                        <label for="varchar">Nama Yang Meninggal <?php echo form_error('nama_yang_meninggal') ?></label>
                        <input type="text" class="form-control" name="nama_yang_meninggal" id="nama_yang_meninggal" placeholder="Nama Yang Meninggal" />
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" name="faktor_kematian" id="faktor_kematian" placeholder="Faktor Kematian" />
                    </div>
                </div>
                <div id="dispensasi">
                    <div class="form-group">
                        <label for="date">Tanggal Dispensasi <?php echo form_error('tanggal_dispensasi') ?></label>
                        <input type="date" class="form-control" name="tanggal_dispensasi" id="tanggal_dispensasi" placeholder="Tanggal Dispensasi" />
                    </div>
                    <div class="form-group">
                        <label for="date">Sampai Tanggal <?php echo form_error('sampai_tanggal_dispensasi') ?></label>
                        <input type="date" class="form-control" name="sampai_tanggal_dispensasi" id="sampai_tanggal_dispensasi" placeholder="Sampai Tanggal Dispensasi" />
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" rows="3" name="alasan_dispen" id="alasan_dispen" placeholder="Alasan Dispen"></textarea>
                    </div>
                </div>
                <div class="form-group">
                    <label for="varchar">RT Domisili <?php echo form_error('rt_domisili') ?></label>
                    <input type="text" class="form-control" name="rt_domisili" id="rt_domisili" placeholder="RT Domisili" required="" value="<?= $warga->rt ?>" />
                </div>
                <button type="submit" class="btn btn-primary" onclick="return confirm('Anda yakin data sudah benar ?')" >Ajukan</button> 
                <a href="<?php echo site_url('pengajuan_surat') ?>" class="btn btn-danger">Batal</a>
            </form>
        </div>
    </div>
</section>
